...
export async function handle(msg, body, client) {
  ...
  if (body.startsWith('.translate ')) {
    ...
    try {
      const result = await askAI(`[seed:${Math.random().toString(36).slice(2,6)}] Translate this text to ${lang}. Reply with ONLY the translation, nothing else:\n"${text}"`);
  ...
  if (body === '.news' || body.startsWith('.news ')) {
    ...
    try {
      ...
      try {
        const news = await askAI(`[seed:${Math.random().toString(36).slice(2,6)}] Give me 5 ${endpoint} news headlines. Numbered list only.`);
        await client.sendMessage(chatId, apexWrap(`📰 *${endpoint.toUpperCase()} News*\n\n${news}`));
      } catch {
