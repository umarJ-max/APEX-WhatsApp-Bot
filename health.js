...
export async function handle(msg, body, client) {
  ...
  if (body.startsWith('.calories ')) {
    ...
    try {
      const result = await askAI(`[seed:${Math.random().toString(36).slice(2,6)}] Give me the approximate calorie count and basic nutrition info (protein, carbs, fat) for: "${food}". Keep it short and formatted with emojis. Just the facts.`);
